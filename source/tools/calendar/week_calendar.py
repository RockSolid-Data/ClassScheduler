...
# Purpose: Week calendar view with 7-day columns and hourly time slots
# Created: 2025-11-13
...
class WeekCalendar(Container):
    """
    Week calendar component that displays 7 days as columns with hourly time slots.
    ...
    def __init__(self, ctx, smgr, window, parent, start_date=None):
        ...
        self.day_headers = {}    # Store day header labels
        ...
        self.week_config = {
            ...
            'hour_height': 80,    # Height per hour slot (increased for better visibility)
            'time_label_width': 70,
            'header_height': 28,
            'colors': {
                'header_bg': 0xE0E0E0,
    ...
    def _create_scrollbar(self):
        """Create scrollbar and scroll buttons for week calendar"""
        # Get container size
    ...
    def _create_week_grid(self):
        """Create the week grid with time slots and day columns"""
        ...
        # Get configuration
        time_label_width = self.week_config['time_label_width']
        hour_height = self.week_config['hour_height']
        start_hour = self.week_config['start_hour']
        end_hour = self.week_config['end_hour']
        header_height = self.week_config['header_height']
        
        # Calculate available width for day columns
        _, _, container_width, _ = self.parent._get_view_container_bounds()
        scrollbar_space = 25
        available_width = container_width - time_label_width - scrollbar_space
        day_column_width = available_width // 7
        ...
        # Create day headers - TWO ROWS (day name + date number, like month view)
        ...
        self.day_headers["lblCornerHeader"] = corner_header  # Store but DON'T add to _base_positions
        
        for day_idx in range(7):
            current_day = week_start + timedelta(days=day_idx)
            day_name = days[day_idx]
            day_number = current_day.strftime("%d")
            
            header_x = time_label_width + (day_idx * day_column_width)
            
            # Highlight today
            is_today = current_day.date() == datetime.now().date()
            bg_color = self.week_config['colors']['today_header_bg'] if is_today else self.week_config['colors']['header_bg']
            text_color = 0xFFFFFF if is_today else 0x333333
            
            # Row 1: Day name
            control_name_row1 = f"lblDayName{day_idx}"
            ...
            self.day_headers[control_name_row1] = header_row1
            
            # Row 2: Date number
            control_name_row2 = f"lblDayDate{day_idx}"
            ...
            self.day_headers[control_name_row2] = header_row2
        ...
        hour_overlaps = self._analyze_hour_overlaps(week_start, start_hour, end_hour)
        ...
        for hour in range(start_hour, end_hour + 1):
            ...
            grid_width = time_label_width + (7 * day_column_width)
        ...
        # Create vertical lines for day columns
        for day_idx in range(8):  # 8 lines for 7 columns (left and right edges + dividers)
            line_x = time_label_width + (day_idx * day_column_width)
            vert_line_name = f"lineVert{day_idx}"
        ...
        # STEP 3: Position items in time slots (now uses pre-calculated hour_positions)
        self._position_items_in_timeslots(week_start, time_label_width, day_column_width, 
        ...
        if self.scrollbar:
            ...
            if total_content_height > visible_height:
                ...
                if self.logger:
                    self.logger.info(f"Week scrollbar enabled: content {total_content_height}px, visible {visible_height}px")
    ...
    def _analyze_hour_overlaps(self, week_start, start_hour, end_hour):
        """
        Analyze how many events overlap in each hour slot across all days.
        Returns dict: {hour: max_overlaps_count}
        """
        hour_overlaps = {}
        
        for hour in range(start_hour, end_hour + 1):
            max_overlaps_for_this_hour = 0
            ...
            for day_idx in range(7):
                ...
                # Track maximum overlaps for this hour across all days
                if overlaps_in_this_slot > max_overlaps_for_this_hour:
                    max_overlaps_for_this_hour = overlaps_in_this_slot
            
            hour_overlaps[hour] = max_overlaps_for_this_hour
        
        return hour_overlaps
    
    def _position_items_in_timeslots(self, week_start, time_label_width, day_column_width, 
                                    time_slot_start_y, start_hour):
        ...
        end_hour = self.week_config['end_hour']
        item_height = 20  # Fixed height for each event (same as month view)
        item_spacing = 2  # Spacing between stacked events
        
        # hour_positions is already calculated in _create_week_grid
        
        # Position items for each day
        for day_idx in range(7):
            current_day = week_start + timedelta(days=day_idx)
            date_str = current_day.strftime("%Y-%m-%d")
            ...
            items = self.calendar_data[date_str]
            slot_x = time_label_width + (day_idx * day_column_width)
            ...
            for hour, items_in_hour in items_by_hour.items():
                ...
                for item_idx, item in enumerate(items_in_hour):
                    ...
                    # Get item color
                    item_color = self.week_config['colors']['item_default']
                    if 'color' in item:
                    ...
                    # Create item button
                    item_id = item.get('id', f"item_{day_idx}_{hour}_{item_idx}")
    ...
    def on_scroll(self, ev):
        ...
        for control_name, (base_x, base_y, width, height) in self._base_positions.items():
            # Skip if this is somehow a header (extra safety check)
            if control_name in self.day_headers:
                continue
    ...
    def scroll_up(self, event):
        ...
        try:
            ...
            new_value = max(min_value, current_value - self.week_config['hour_height'])
    ...
    def scroll_down(self, event):
        ...
        try:
            ...
            new_value = min(max_value, current_value + self.week_config['hour_height'])
    ...
    def navigate_prev(self):
        ...
        if self.logger:
            self.logger.info("Navigate to previous week")
    ...
    def navigate_next(self):
        ...
        if self.logger:
            self.logger.info("Navigate to next week")
        self.current_date = self.current_date + timedelta(days=7)
        self.reload_data()
    
    def reload_data(self):
        """Reload week data and recreate the grid"""
        # Reload week data
        self.load_week_data()
        
        # Recreate the week grid
        self._create_week_grid()
    
    def load_week_data(self):
        """Load calendar items for the current week"""
        try:
            ...
            self.calendar_data = self.parent.get_items_callback(week_start, week_end, self.parent.selected_filter)
            
            if self.logger:
                self.logger.info(f"Loading week data for {week_start} to {week_end}")
                self.logger.info(f"Found {len(self.calendar_data)} days with items")
                
        except Exception as e:
            if self.logger:
                self.logger.error(f"Error loading week data: {e}")
                self.logger.error(traceback.format_exc())
            self.calendar_data = {}
    
    def resize(self, width, height):
        """Handle container resize events - recalculate and recreate week grid"""
        try:
            if self.logger:
                self.logger.info(f"Resizing WeekCalendar to {width}x{height}")
            ...
            # Recreate the week grid with new dimensions
            self._create_week_grid()
            
            if self.logger:
                self.logger.info("WeekCalendar resize complete")
            
        except Exception as e:
            if self.logger:
                self.logger.error(f"Error during WeekCalendar resize: {e}")
                self.logger.error(traceback.format_exc())
    ...
    def print_calendar(self, event=None):
        """
        Generate a Writer document representing the week calendar and open print dialog.
        This method can be called directly or used as a button callback.
        """
        try:
            if self.logger:
                self.logger.info("Printing week calendar...")
            ...
            title_text = f"Week of {week_start.strftime('%B %d, %Y')} - {week_end.strftime('%B %d, %Y')}"
            ...
            # Create table (time column + 7 day columns, hours + 1 header row)
            table = doc.createInstance("com.sun.star.text.TextTable")
            table.initialize(num_hours + 1, 8)  # +1 for header row, 8 columns (time + 7 days)
            ...
            for col_idx, day in enumerate(days):
                ...
                # Format for day headers (not time column)
                if col_idx > 0:
                    day_date = week_start + timedelta(days=col_idx - 1)
                    is_today = day_date.date() == datetime.now().date()
                    
                    if is_today:
                        cell.BackColor = 0x3498DB
                        cell_cursor.CharColor = 0xFFFFFF
                    else:
                        cell.BackColor = 0xE0E0E0
                        cell_cursor.CharColor = 0x333333
                    
                    # Add day name and date
                    cell.setString(f"{day}\n{day_date.strftime('%m/%d')}")
                else:
                    # Time column header
                    cell.BackColor = 0xE0E0E0
                    cell_cursor.CharColor = 0x333333
            ...
            for hour_idx in range(num_hours):
                ...
                for day_idx in range(7):
                    col_num = day_idx + 1
                    cell = table.getCellByPosition(col_num, row_num)
                    
                    # Get items for this day and hour
                    current_day = week_start + timedelta(days=day_idx)
                    date_str = current_day.strftime('%Y-%m-%d')
                    items = self.calendar_data.get(date_str, [])
                    
                    # Filter items for this specific hour
                    hour_items = []
                    for item in items:
                        start_time = item.get('start_time')
                        if start_time and isinstance(start_time, datetime):
                            if start_time.hour == hour:
                                hour_items.append(item)
            ...
            # Set column widths (time column narrower than day columns)
            try:
                columns = table.getColumns()
                if columns is not None:
                    # Time column gets 1 unit, day columns get 2 units each
                    for i in range(8):
                        col = columns.getByIndex(i)
                        if col is not None:
                            if i == 0:
                                col.setPropertyValue("RelativeWidth", 1)  # Time column
                            else:
                                col.setPropertyValue("RelativeWidth", 2)  # Day columns
            except Exception as col_err:
                if self.logger:
                    self.logger.debug(f"Could not set column widths: {col_err}")
            
            if self.logger:
                self.logger.info("Week calendar document created successfully")
            ...
            if self.logger:
                self.logger.error(f"Error printing week calendar: {e}")
                self.logger.error(traceback.format_exc())
            
            # Try to show error message to user
            try:
                from librepy.pybrex.msgbox import msgbox
                msgbox(f"Error printing week calendar:\n{str(e)}", "Print Error")
            except:
                pass
    ...
    def dispose(self):
        ...
        try:
            if self.logger:
                self.logger.info("Disposing of WeekCalendar")
